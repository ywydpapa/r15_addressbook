...
// 1. 클럽별 행사 목록 화면
// =========================================================
class ClubEventManageScreen extends StatefulWidget {
  final int clubNo;
  final String clubName;
  final String memberNo;

  const ClubEventManageScreen({
    super.key,
  ...
  @override
  State<ClubEventManageScreen> createState() => _ClubEventManageScreenState();
}

class _ClubEventManageScreenState extends State<ClubEventManageScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${widget.clubName} 행사 목록'),
...
// =========================================================
// 2. 클럽 행사 추가 BottomSheet 입력 폼 위젯
// =========================================================
class AddClubEventBottomSheet extends StatefulWidget {
  final int clubNo;
  final VoidCallback onSuccess;

  const AddClubEventBottomSheet({
    super.key,
    required this.clubNo,
  ...
  @override
  State<AddClubEventBottomSheet> createState() => _AddClubEventBottomSheetState();
}

class _AddClubEventBottomSheetState extends State<AddClubEventBottomSheet> {
  ...
  Future<void> _submitEvent() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('${ApiConf.baseUrl}/phapp/insertclubEvent'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('클럽 행사가 성공적으로 추가되었습니다.')),
        );
        widget.onSuccess();
...
// =========================================================
// 3. 클럽 행사별 참석자 명단 및 상세 응답 관리 화면
// =========================================================
class ClubEventAttendeeScreen extends StatefulWidget {
  final int eventNo;
  final String eventTitle;
  final String memberNo;

  const ClubEventAttendeeScreen({
    super.key,
  ...
  @override
  State<ClubEventAttendeeScreen> createState() => _ClubEventAttendeeScreenState();
}

class _ClubEventAttendeeScreenState extends State<ClubEventAttendeeScreen> {
  ...
  Future<void> _fetchMyAttendance() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
          '${ApiConf.baseUrl}/phapp/club/event/my-attend?eventNo=${widget.eventNo}&memberNo=${widget.memberNo}',
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
  ...
  Future<void> _fetchAttendees() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${ApiConf.baseUrl}/phapp/club/event/members/${widget.eventNo}'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  Future<void> _saveAttendance({
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('${ApiConf.baseUrl}/phapp/club/event/attend'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
